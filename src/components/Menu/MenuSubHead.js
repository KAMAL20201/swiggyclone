...
function MenuSubHead(props) {
  ...
  return (
    ...
    <OffersCards>
      {RestInfo?.cards[1].card?.card?.gridElements?.infoWithStyle?.offers?.map((item) => {
        return (
          <Cards key={item?.info?.offerIds}>
            <h3><img height="12px" alt="" src={restaurantCardURL+item?.info?.offerLogo}/>{item?.info?.header}</h3>
